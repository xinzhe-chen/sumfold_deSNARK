...
# nMIN must be large enough so N/K >= 1 (i.e. nMIN >= log2(K))
(( NV_MIN >= LOG_K )) || die "nMIN ($NV_MIN) must be >= log2(k) = $LOG_K (constraints per party must be >= 1)"
...
echo -e "${YELLOW}Starting benchmark: nv=${NV_MIN}..${NV_MAX}, K=${K}, M=${M}${NC}"
echo ""
...
WORKER_PIDS=()
for (( i = 1; i < K; i++ )); do
    "$BIN" --party $i --nv-min $NV_MIN --nv-max $NV_MAX "$CONFIG_FILE" \
        > "$LOG_DIR/p${i}.log" 2>&1 &
    WORKER_PIDS+=($!)
done
sleep 2
...
echo -e "${GREEN}Running master (party 0)...${NC}"
"$BIN" --party 0 --nv-min $NV_MIN --nv-max $NV_MAX "$CONFIG_FILE" \
