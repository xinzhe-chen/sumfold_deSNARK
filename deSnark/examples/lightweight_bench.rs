...
/// Returns (user_ms, sys_ms) — cumulative CPU time for this process since start.
...
impl PeakSampler {
    fn start(initial_rss: f64) -> Self {
        ...
        let result = Arc::new(Mutex::new(PeakSample { peak_rss_mb: initial_rss, peak_cpu_pct: 0.0 }));
        ...
        let handle = thread::spawn(move || {
            ...
            while !flag.load(Ordering::Relaxed) {
                ...
                if let Some(p) = sys.process(pid) {
                    ...
                    if rss_mb  > guard.peak_rss_mb  { guard.peak_rss_mb  = rss_mb; }
                    if cpu_pct > guard.peak_cpu_pct { guard.peak_cpu_pct = cpu_pct; }
                }
                thread::sleep(Duration::from_millis(5));
            }
        });

        Self { stop_flag, result, handle: Some(handle) }
    }
    ...
    fn stop(mut self) -> PeakSample {
        ...
        PeakSample { peak_rss_mb: g.peak_rss_mb, peak_cpu_pct: g.peak_cpu_pct }
    }
}
    ...
    F: FnOnce() -> T,
{
    ...
    (result, PhaseStats { wall_ms, avg_cpu_pct, peak_cpu_pct, peak_rss_mb })
}
...
fn bench_config(log_m: usize, log_nv: usize, log_k: usize) -> ConfigResult {
    ...
    let ((pk, _vk, circs), s1) = timed(|| make_circuit::<Bn254, PCS>(&config, &srs).expect("make_circuit"));
...
fn fmt_pct(v: f64) -> String {
    if v.is_nan() { "  —  ".to_string() } else { format!("{:>6.0}%", v) }
}

fn fmt_ms(v: f64) -> String {
    if v < 1.0 { format!("{:>8.3}", v) } else { format!("{:>8.1}", v) }
}
...
fn print_results(results: &[ConfigResult], total_ram: f64) {
    ...
    for r in results {
        ...
        println!(
            ...
            r.log_m, r.log_nv, r.log_k, r.m, r.k, r.nv_eff, r.total_ms,
            "",
    ...
    // ── Summary table ──────────────────────────────────────────────────────
    println!("╔{}╗", bar);
    println!("║{:^width$}║", "  Summary — prove_sumfold  (core proving work)", width = w);
    println!("╠{}╣", bar);
    println!(
        "║  {:<18}  {:>4}  {:>4}  {:>6}  {:>10}  {:>9}  {:>10}  {:>10}  ║",
    ...
    for r in results {
        ...
        println!(
            ...
            r.m, r.k, r.nv_eff,
            s.wall_ms, fmt_pct(s.avg_cpu_pct), fmt_pct(s.peak_cpu_pct), s.peak_rss_mb,
        );
    }
    println!("╚{}╝", bar);
    println!();
    println!("  System RAM   : {:.1} MB ({:.1} GB)", total_ram, total_ram / 1024.0);
...
fn main() {
    ...
    eprintln!("deSNARK lightweight bench  ({} Rayon threads, {:.1} GB RAM)",
        rayon::current_num_threads(), total_ram / 1024.0);
    eprintln!();

    let mut results = Vec::with_capacity(configs.len());
    for &(log_m, log_nv, log_k) in configs {
        eprintln!("  running ({},{},{})  M={}  K={}  nv_eff={}...",
            log_m, log_nv, log_k, 1 << log_m, 1 << log_k, log_nv - log_k);
