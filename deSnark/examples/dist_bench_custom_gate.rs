...
fn main() {
    ...
    let (mut base_config, mut net_config) = Config::from_toml_file(&cli.config_path)
        .unwrap_or_else(|e| {
    ...
    for nv in cli.nv_min..=cli.nv_max {
        ...
        match dist_prove::<Bn254>(&config) {
            ...
            Err(e) => {
                error!("dist_prove warmup failed at nv={}: {}", nv, e);
                continue;
            },
        }
        ...
        for rep in 0..cli.repetitions {
            ...
            match dist_prove::<Bn254>(&config) {
                ...
                Err(e) => {
                    let _ = sampler.stop();
                    error!("dist_prove failed at nv={}, rep={}: {}", nv, rep + 1, e);
                    break;
                },
            }
        }
...
fn print_usage(prog: &str) {
    ...
    eprintln!(
        "  --mock-degree <D>       Mock gate degree (default: 3, only for --gate-preset mock)"
    );
