...
impl GateType {
    ...
    ///
    /// # Panics
    /// Panics if `Mock` parameters are invalid. Call [`validate()`](Self::validate)
    /// first when constructing from untrusted input (e.g. TOML config).
...
mod tests {
    ...
    fn test_gate_type_to_gate() {
        ...
        // mock_gate(n, d) creates a high-degree term with d witness factors + selector → degree d+1
