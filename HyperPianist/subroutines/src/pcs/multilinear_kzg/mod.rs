...
impl<E: Pairing> PolynomialCommitmentScheme<E> for MultilinearKzgPCS<E> {
    ...
    fn commit(
        ...
        let commitment = E::G1MSM::msm_unchecked_par_auto(
...
fn open_internal<E: Pairing>(
        ...
        .enumerate()
    {
        ...
        let k = nv - 1 - i;
        let cur_dim = 1 << k;
        let mut q = unsafe_allocate_zero_vec::<E::ScalarField>(cur_dim);
        let mut r = unsafe_allocate_zero_vec::<E::ScalarField>(cur_dim);
        ...
        proofs.push(E::G1MSM::msm_unchecked(&gi.evals, &q).into());
        end_timer!(msm_timer);
