...
#[derive(PartialEq, Eq, Hash, Default, CanonicalSerialize, CanonicalDeserialize)]
...
impl<F: Field> Clone for DenseMultilinearExtension<F> {
    fn clone(&self) -> Self {
        let size = self.evaluations.len();
        let result: Vec<F>;
        unsafe {
            let layout = ark_std::alloc::Layout::array::<F>(size).unwrap();
            let ptr = ark_std::alloc::alloc(layout) as *mut F;

            if ptr.is_null() {
                panic!("Vec allocation failed");
            }

            ark_std::ptr::copy_nonoverlapping(self.evaluations.as_ptr(), ptr, layout.size());
