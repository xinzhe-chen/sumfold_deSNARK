//! HyperPianist DeMkzg benchmark — Custom Gate variant
//!
//! Identical to `bench.rs` but supports selecting the gate type via CLI
//! and emits an extra `gate_name` column in the CSV output.
//!
//! ## Usage
//!
//! ```bash
//! hyperpianist-bench-custom-gate <party_id> <hosts_file> <num_vars> [gate_preset]
//!     [--mock-degree D] [--mock-num-witness W]
//! ```
//!
//! gate_preset: vanilla (default), jellyfish_turbo, super_long_selector, mock
...
#[structopt(name = "hyperpianist-bench-custom-gate", about = "HyperPianist DeMkzg benchmark with custom gate support")]
struct Opt {
    /// Party id (0 = master)
    id: usize,
    ...
    /// Gate preset: vanilla, jellyfish_turbo, super_long_selector, mock
    #[structopt(default_value = "vanilla")]
    gate_preset: String,

    /// Mock gate degree (only used when gate_preset = "mock")
    #[structopt(long, default_value = "3")]
    mock_degree: usize,

    /// Mock gate num witnesses (only used when gate_preset = "mock")
    #[structopt(long, default_value = "4")]
    mock_num_witness: usize,
}

fn parse_gate(opt: &Opt) -> (CustomizedGates, String) {
    match opt.gate_preset.as_str() {
        "vanilla" => (CustomizedGates::vanilla_plonk_gate(), "vanilla".to_string()),
        "jellyfish_turbo" => (CustomizedGates::jellyfish_turbo_plonk_gate(), "jellyfish_turbo".to_string()),
        "super_long_selector" => (CustomizedGates::super_long_selector_gate(), "super_long_selector".to_string()),
        "mock" => {
            if opt.mock_degree < 1 {
                eprintln!("Error: --mock-degree must be >= 1, got {}", opt.mock_degree);
                std::process::exit(1);
            }
            if opt.mock_num_witness < 1 {
                eprintln!("Error: --mock-num-witness must be >= 1, got {}", opt.mock_num_witness);
                std::process::exit(1);
            }
            let gate = CustomizedGates::mock_gate(opt.mock_num_witness, opt.mock_degree);
            let name = format!("mock_w{}_d{}", opt.mock_num_witness, opt.mock_degree);
            (gate, name)
        },
        other => {
...
fn main() {
    ...
    let (gate, gate_name) = parse_gate(&opt);

    if Net::am_master() {
        eprintln!(
            "# HyperPianist custom-gate benchmark: gate={}, nv={}, K={}",
            gate_name, opt.num_vars, Net::n_parties(),
        );
        eprintln!(
            "# Gate info: degree={}, witnesses={}, selectors={}",
            gate.degree(), gate.num_witness_columns(), gate.num_selector_columns(),
        );
        eprintln!(
    ...
    Helper::<DeMkzg<Bn254>>::bench_with_gate(&pcs_srs, opt.num_vars, &gate, &gate_name).unwrap();
    ...
    >,
{
    fn bench_with_gate(
        ...
        // Format: setup_ms,prover_ms,verifier_ms,proof_bytes,comm_sent,comm_recv,cpu_ms,wall_ms,peak_rss_mb,gate_name
        if Net::am_master() {
            println!(
                "{:.3},{:.3},{:.3},{},{},{},{:.3},{:.3},{:.1},{}",
