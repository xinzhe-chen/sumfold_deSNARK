    ...
    pub fn new(num_constraints: usize, gate: &CustomizedGates) -> MockCircuit<F> {
        ...
        for cs in 0..num_constraints {
            let mut cur_selectors: Vec<F> = (0..(num_selectors - 1))
                ...
                .collect();
            let cur_witness: Vec<F> = if cs < num_constraints / 4 {
                (0..num_witnesses).map(|_| F::rand(&mut rng)).collect()
            } else {
        ...
        let permutation = (0..num_witnesses)
            .flat_map(|witness_idx| {
                let portion_len = num_constraints / 4;
                (0..portion_len)
                    .map(|i| {
                        F::from_u64((witness_idx * num_constraints + i + portion_len) as u64)
                            .unwrap()
                    })
                    .chain((0..portion_len).map(|i| {
                        F::from_u64((witness_idx * num_constraints + i + 3 * portion_len) as u64)
                            .unwrap()
                    }))
                    .chain(
                        (0..portion_len).map(|i| {
                            F::from_u64((witness_idx * num_constraints + i) as u64).unwrap()
                        }),
                    )
                    .chain((0..portion_len).map(|i| {
                        F::from_u64((witness_idx * num_constraints + i + 2 * portion_len) as u64)
                            .unwrap()
                    }))
                    .collect::<Vec<_>>()
            })
        ...
        let permutation = (0..num_witnesses)
            .flat_map(|witness_idx| {
                ...
                (0..portion_len)
                    ...
                    .chain((0..portion_len).map(|i| {
                        F::from_u64(
                            (witness_idx * total_num_constraints + shift + i + 3 * portion_len)
                                as u64,
                        )
                        .unwrap()
                    }))
                    .chain((0..portion_len).map(|i| {
                        F::from_u64((witness_idx * total_num_constraints + shift + i) as u64)
                            .unwrap()
                    }))
                    .chain((0..portion_len).map(|i| {
                        F::from_u64(
                            (witness_idx * total_num_constraints + shift + i + 2 * portion_len)
                                as u64,
                        )
                        .unwrap()
                    }))
