...
//!
//! This module contains the implementation of the Dory inner product argument
...
//!
//! Note: from the paper:
//! > In our initial presentation of the protocols, and discussions of
//! > completeness
//! > and soundness, we will highlight that which is required only to achieve
//! > hiding
//! > in commitments and zero-knowledge in the protocols in blue.
//!
//! This implementation only implements the computational integrity component of
//! Dory. This can be extended in the future to achieve hiding, but that isn't
//! needed for our initial use-case.

// This is so that the naming in the code more closely matches the naming in the paper, since the
// paper used both capital and non-capital letters.
...
mod base;
type DeferredGT<E> = base::deferred_msm::DeferredMSM<PairingOutput<E>, <E as Pairing>::ScalarField>;
type DeferredG1<E> =
    base::deferred_msm::DeferredMSM<<E as Pairing>::G1Affine, <E as Pairing>::ScalarField>;
type DeferredG2<E> =
    base::deferred_msm::DeferredMSM<<E as Pairing>::G2Affine, <E as Pairing>::ScalarField>;

pub mod setup;
pub use setup::ProverSetup;
pub mod deSetup;
pub use deSetup::{PublicParameters, SubProverSetup, VerifierSetup};

mod commit;
pub use commit::DoryCommitment;
mod deCommit;
pub use deCommit::DeDoryCommitment;
...
// #[cfg(test)]
// pub(crate) use dory_inner_product::{dory_inner_product_prove,
// dory_inner_product_verify};
...
// pub(crate) use extended_dory_reduce::{extended_dory_reduce_prove,
// extended_dory_reduce_verify}; pub(crate) use
// fold_scalars::{fold_scalars_0_prove, fold_scalars_0_verify};
...
// mod dory_vmv_helper;
// use dory_vmv_helper::{
//     compute_L_R_vec, compute_T_vec_prime, compute_l_r_tensors, compute_nu,
// compute_v_vec, };
// mod build_vmv_state;
